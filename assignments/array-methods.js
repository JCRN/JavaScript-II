...
runners.forEach(runner => {
    ++acc;
    announceRunner.push(`In the number ${acc} slot: ${runner.first_name} ${runner.last_name} running for ${runner.company_name}.`);
})
console.log(announceRunner);

// Problem 2
let eduEmail = []
runners.filter((runner) => {
    if(runner.email.slice(-3) === 'edu') {
        eduEmail.push(`${runner.first_name} ${runner.last_name} ${runner.email}`);
    }
});
...
let lastNamesAtoN = []
runners.map(runner => {
    let comp = runner.last_name[0];
    if(comp < 'O') {
        lastNamesAtoN.push(`${runner.last_name}`);
    }
    lastNamesAtoN.sort();
})
console.log(lastNamesAtoN);